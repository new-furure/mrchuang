  ...
  function icheckContent(str){
  if(str==""){      //当用户名为空时
    //alert("昵称不能为空！");
    //document.post_form.inputNickname.focus();
    document.getElementById("div_improve").innerHTML="请输入内容.";//设置提示文字
    document.getElementById("tr_improve").style.display='block'; //显示提示信息

  }
  else{    //进行异步操作，判断用户名是否被注册
    document.getElementById("div_improve").innerHTML="";
    document.getElementById("tr_improve").style.display='none';
    //var loader=new net.AjaxRequest("UserServlet?action=checkUser&username="+str+"&nocache="+new Date().getTime(),deal,onerror,"GET");
  } 
}

function ipost_submit(){
  var flag_content=true;

  if(improve_form.inputImprove.value==""){   //当用户名为空时
     document.getElementById("div_improve").innerHTML="请输入内容.";//设置提示文字
    document.getElementById("tr_improve").style.display='block'; //显示提示信息
    flag_content=false;
  }  
  ...
  var newDigi = document.getElementById("improve_submit"); 
  var article_id=newDigi.getAttribute("aid");


  
  var newDigi2 = document.getElementById("improve_submit");

  var content=$('textarea[name=inputImprove]');
  
      if (content!= "") {

        var a=$.post(handleUrlRA,{comment_content:content.val(),article_id:article_id,comment_type:improve_type},function(data){
          if(data.type=0)
            alert('评论失败，请重试');
          else{
